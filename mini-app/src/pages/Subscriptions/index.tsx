...
export default function Subscriptions() {
    const { configs, devices, fetchAll } = useUserStore();
    ...
    const handleAutoConnect = async () => {
        if (!activeConfig) {
            window.Telegram?.WebApp?.showAlert("Ключ еще не создан. Перейдите в профиль.");
            return;
        }
    ...
    return (
        <div className="flex flex-col h-[78vh] overflow-y-auto custom-scrollbar px-1 pb-32 pt-2 animate-in fade-in duration-500">
            ...
            <div className="bg-[#12141d] border border-white/10 rounded-[2.5rem] p-6 mb-4 shadow-xl relative overflow-hidden shrink-0">
                ...
                {step === 2 && (
                    <div className="text-center animate-in slide-in-from-right-4">
                        ...
                        <h3 className="text-[20px] font-black text-white uppercase italic mb-3">Шаг 2. Установка</h3>
                        <p className="text-[14px] text-white/50 leading-relaxed mb-8 px-4">
                            Запустите приложение и разрешите установку VPN-профиля в системе.
                        </p>
                    ...
                    <div className="animate-in zoom-in-95">
                        ...
                        >
                            <div className="flex items-center gap-2">
                                ...
                                <span>{isConnecting ? 'ОТКРЫВАЕМ...' : 'АВТО-ПОДКЛЮЧЕНИЕ'}</span>
                            </div>
                        </button>
                        ...
                        >
                            <div className="flex items-center gap-3">
                                ...
                                <span className="text-[13px] font-bold text-white/80">Скопировать ссылку</span>
                            </div>
                            {copyStatus ? (
                                <span className="text-[10px] font-black text-emerald-500 uppercase">Скопировано</span>
                            ) : (
            ...
            {/* ИНСТРУКЦИЯ (ВИДНА ТОЛЬКО НА ШАГЕ 3) */}
            {step === 3 && (
                <div className="bg-[#12141d] border border-white/10 rounded-[2.5rem] p-6 shrink-0 shadow-lg animate-in slide-in-from-bottom-4">
                    <div className="flex items-center gap-2 mb-5">
                        ...
                        <h4 className="text-[11px] font-black text-white uppercase tracking-[0.1em]">Инструкция для Happ</h4>
                    </div>
            ...
            {/* ИНДИКАТОРЫ ШАГОВ ВНИЗУ */}
            <div className="mt-auto pt-8 flex justify-center items-center gap-2 shrink-0">
                {[1, 2, 3].map((i) => (
                    <div
                        key={i}
                        className={`h-1 rounded-full transition-all duration-500 ${step === i ? 'w-8 bg-white' : 'w-2 bg-white/10'}`}
                    />
                ))}
            </div>
        </div>
