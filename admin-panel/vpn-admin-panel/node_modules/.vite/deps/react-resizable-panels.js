...
function Se(e, t) {
	return Array.from(t).sort(e === "horizontal" ? xt : bt);
}
function xt(e, t) {
...
function Ye(e) {
	const { element: t, orientation: n, panels: o, separators: i } = e, s = Se(n, Array.from(t.children).filter(Xe).map((b) => ({ element: b }))).map(({ element: b }) => b), l = [];
	let r = !1, a = !1, u = -1, c = -1, m = 0, d, v = [];
	{
		let b = -1;
		for (const f of s) f.hasAttribute("data-panel") && (b++, f.ariaDisabled === null && (m++, u === -1 && (u = b), c = b));
	}
	if (m > 1) {
		let b = -1;
...
function se({ delta: e, initialLayout: t, panelConstraints: n, pivotIndices: o, prevLayout: i, trigger: s }) {
		...
		case "keyboard":
			{
				...
				if (y) {
					const S = r[f];
					if (L(S != null, `Previous layout not found for panel index ${f}`), k(S, g)) {
						const p = z - S;
			...
			}
			{
				...
				if (y) {
					const S = r[f];
					if (L(S != null, `Previous layout not found for panel index ${f}`), k(S, z)) {
						const p = S - g;
						G(p, Math.abs(e)) > 0 && (e = e < 0 ? 0 - p : p);
					}
				}
	...
	}
	{
		...
		for (; h >= 0 && h < n.length;) {
			...
			if (!k(y, S) && (d += y - S, u[h] = S, d.toFixed(3).localeCompare(Math.abs(e).toFixed(3), void 0, { numeric: !0 }) >= 0)) break;
			e < 0 ? h-- : h++;
		}
	}
	if (Ot(a, u)) return i;
	{
		...
		if (u[f] = y, !k(y, g)) {
			...
			for (; p >= 0 && p < n.length;) {
				...
				if (k(P, C) || (z -= C - P, u[p] = C), k(z, 0)) break;
				e > 0 ? p-- : p++;
			}
		}
	}
	if (!k(Object.values(u).reduce((f, h) => h + f, 0), 100, .1)) return i;
...
function B({ layout: e, panelConstraints: t }) {
	...
	if (!k(s, 0)) for (let r = 0; r < t.length; r++) {
		...
		if (a !== c && (s -= c - a, o[r] = c, k(s, 0))) break;
	}
...
function et({ groupId: e, panelId: t }) {
	const n = () => {
		...
		const a = s();
		if (r === a) return;
		const { defaultLayoutDeferred: u, derivedPanelConstraints: c, group: m, groupSize: d, layout: v, separatorToPanels: b } = n(), f = m.panels.findIndex((z) => z.id === t), h = f === m.panels.length - 1, y = B({
			layout: se({
				delta: h ? a - r : r - a,
				initialLayout: v,
				panelConstraints: c,
...
function Ut({ children: e, className: t, defaultLayout: n, disableCursor: o, disabled: i, elementRef: s, groupRef: l, id: r, onLayoutChange: a, onLayoutChanged: u, orientation: c = "horizontal", resizeTargetMinimumSize: m = {
	...
	}), g = be(r), y = (0, import_react.useRef)(null), [z, S] = Bt(), p = (0, import_react.useRef)({
	...
	const M = oe((x, w) => {
		...
		if (I) {
			...
			switch (E.state) {
				case "active":
					O = E.hitRegions.some((H) => H.group === R);
					break;
			}
			return {
		...
		registerPanel: (x) => {
			const w = p.current;
			return w.panels = Se(c, [...w.panels, x]), S(), () => {
				w.panels = w.panels.filter((E) => E !== x), S();
			};
		},
		registerSeparator: (x) => {
			const w = p.current;
			return w.separators = Se(c, [...w.separators, x]), S(), () => {
				w.separators = w.separators.filter((E) => E !== x), S();
			};
		},
		togglePanelDisabled: (x, w) => {
			const R = p.current.panels.find((H) => H.id === x);
		...
		toggleSeparatorDisabled: (x, w) => {
			const R = p.current.separators.find((I) => I.id === x);
			R && (R.disabled = w);
		}
	}), [
		M,
		...
		const lt = ye(g, (ee) => {
			...
			R.mutableState.layouts[ut] = le, Re.forEach((N) => {
				if (N.collapsible) {
					...
					if (pe) {
						const ft = k(N.collapsedSize, le[N.panelId]), dt = k(N.collapsedSize, pe[N.panelId]);
...
function qt({ children: e, className: t, collapsedSize: n = "0%", collapsible: o = !1, defaultSize: i, disabled: s, elementRef: l, groupResizeBehavior: r = "preserve-relative-size", id: a, maxSize: u = "100%", minSize: c = "0%", onResize: m, panelRef: d, style: v, ...b }) {
	const f = !!a, h = be(a), g = Pe({ disabled: s }), y = (0, import_react.useRef)(null), z = we(y, l), { getPanelStyles: S, id: p, orientation: P, registerPanel: M, togglePanelDisabled: C } = Le(), X = m !== null, F = oe((R, I, O) => {
		m?.(R, a, O);
	});
	K(() => {
		const R = y.current;
	...
	}, w = (0, import_react.useSyncExternalStore)((R) => ye(p, R), x, x);
	let E;
...
function Zt({ children: e, className: t, disabled: n, elementRef: o, id: i, style: s, ...l }) {
	const r = be(i), a = Pe({ disabled: n }), [u, c] = (0, import_react.useState)({}), [m, d] = (0, import_react.useState)("inactive"), v = (0, import_react.useRef)(null), b = we(v, o), { disableCursor: f, id: h, orientation: g, registerSeparator: y, toggleSeparatorDisabled: z } = Le(), S = g === "horizontal" ? "vertical" : "horizontal";
	K(() => {
		const P = v.current;
		if (P !== null) {
			const M = {
				...
				d(x.next.state !== "inactive" && x.next.hitRegions.some((w) => w.separator === M) ? x.next.state : "inactive");
			}), F = ye(h, (x) => {
