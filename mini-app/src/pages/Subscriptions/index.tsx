...
import {
    ...
    Loader2, Copy, ExternalLink, HelpCircle, ChevronRight, Info
} from 'lucide-react';

export default function Subscriptions() {
    const { configs, t } = useUserStore();
    ...
    const handleAutoConnect = async () => {
        if (!activeConfig) {
            window.Telegram?.WebApp?.showAlert(t.no_key_alert);
            return;
        }
        setIsConnecting(true);
        window.Telegram?.WebApp?.HapticFeedback.impactOccurred('medium');
        const importUrl = `${activeConfig.subscriptionUrl}/import/happ`;
        if (window.Telegram?.WebApp) {
            window.Telegram.WebApp.openLink(importUrl);
        } else {
    ...
    return (
        <div className="flex flex-col h-[78vh] overflow-y-auto custom-scrollbar px-1 pb-32 pt-2 animate-in fade-in duration-500">
            ...
            <div className="flex justify-between items-center mb-6 px-2 shrink-0">
                ...
                >
                    {t.step_next} <ArrowRight size={14} />
                </button>
            </div>
            ...
            <div className="bg-[#12141d] border border-white/10 rounded-[2.5rem] p-6 mb-4 shadow-xl relative overflow-hidden shrink-0">
                ...
                {step === 1 && (
                    <div className="text-center animate-in slide-in-from-left-4">
                        ...
                        <h3 className="text-[20px] font-black text-white uppercase italic mb-3">{t.step1_title}</h3>
                        <p className="text-[14px] text-white/50 leading-relaxed mb-8 px-4">
                            {t.step1_desc} <span className="text-white font-bold">{platform.app}</span>.
                        </p>
                        <button onClick={() => window.Telegram?.WebApp?.openLink(platform.link)} className="w-full py-4 bg-white text-black rounded-xl font-black text-[13px] uppercase tracking-widest active:scale-[0.98] transition-all flex items-center justify-center gap-2">
                            {t.step1_btn} <ExternalLink size={16} />
                        </button>
                    </div>
                )}
                ...
                {step === 2 && (
                    <div className="text-center animate-in slide-in-from-right-4">
                        ...
                        <h3 className="text-[20px] font-black text-white uppercase italic mb-3">{t.step2_title}</h3>
                        <p className="text-[14px] text-white/50 leading-relaxed mb-8 px-4">{t.step2_desc}</p>
                        <button onClick={() => setStep(3)} className="w-full py-4 bg-white text-black rounded-xl font-black text-[13px] uppercase tracking-widest active:scale-[0.98] transition-all">
                            {t.step2_btn}
                        </button>
                    </div>
                )}
                ...
                {step === 3 && (
                    <div className="animate-in zoom-in-95">
                        <h3 className="text-[20px] font-black text-white uppercase italic mb-6 text-center">{t.step3_title}</h3>
                        ...
                        <button
                            ...
                            className={`w-full py-6 rounded-2xl font-black text-[16px] uppercase tracking-widest flex flex-col items-center justify-center gap-1 transition-all border ${
                                isConnecting ? 'bg-emerald-600 border-emerald-500 text-white' : 'bg-white border-white text-black active:scale-95 shadow-[0_0_30px_rgba(255,255,255,0.1)]'
                            }`}
                        >
                            <div className="flex items-center gap-2">
                                {isConnecting ? <Loader2 size={22} className="animate-spin" /> : <Rocket size={22} />}
                                <span>{isConnecting ? t.connecting : t.auto_connect}</span>
                            </div>
                        </button>

                        <div className="flex items-center gap-3 my-6">
                            <div className="h-[1px] bg-white/5 flex-1" />
                            <span className="text-[10px] font-black text-white/20 uppercase tracking-widest">{t.or_manual}</span>
                            <div className="h-[1px] bg-white/5 flex-1" />
                        </div>
                        ...
                        >
                            <div className="flex items-center gap-3">
                                <Copy size={18} className="text-white/40 group-active:text-white" />
                                <span className="text-[13px] font-bold text-white/80">{t.copy_link}</span>
                            </div>
                            {copyStatus ? (
                                <span className="text-[10px] font-black text-emerald-500 uppercase">{t.copied}</span>
                            ) : (
            ...
            {step === 3 && (
                <div className="bg-[#12141d] border border-white/10 rounded-[2.5rem] p-6 shrink-0 shadow-lg animate-in slide-in-from-bottom-4">
                    <div className="flex items-center gap-2 mb-5">
                        ...
                        <h4 className="text-[11px] font-black text-white uppercase tracking-[0.1em]">{t.guide_title}</h4>
                    </div>

                    <div className="space-y-5">
                        {[
                            { s: '1', text: t.guide_step1 },
                            { s: '2', text: `${t.guide_step2} ${platform.app}.` },
                            { s: '3', text: t.guide_step3 },
                            { s: '4', text: t.guide_step4 },
                        ].map((item, idx) => (
                            <div key={idx} className="flex items-start gap-4">
                                ...
                                <p className="text-[13px] text-white/60 leading-tight">{item.text}</p>
                            </div>
                        ))}
                    </div>
                    ...
                    <div className="mt-6 pt-5 border-t border-white/5 flex items-center gap-3">
                        ...
                        <p className="text-[11px] text-white/30 italic">{t.guide_note}</p>
                    </div>
                </div>
            )}

            <div className="mt-auto pt-8 flex justify-center items-center gap-2 shrink-0">
                {[1, 2, 3].map((i) => (
                    <div key={i} className={`h-1 rounded-full transition-all duration-500 ${step === i ? 'w-8 bg-white' : 'w-2 bg-white/10'}`} />
                ))}
            </div>
        </div>
    );
