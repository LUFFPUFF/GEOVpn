...
import {
    ...
    ShieldCheck, LineChart, Bell, Sun, Moon, Wifi
} from "lucide-react"
...
export default function AdminLayout({ onLogout }: { onLogout: () => void }) {
    ...
    const navItems = [
        ...
        { name: "Конфигурации", path: "/configs", icon: Wifi },
    ...
    return (
        <div className="flex h-screen bg-[#f4f7fb] dark:bg-[#020617] font-sans antialiased transition-colors duration-300">
            {/* GLOBAL CSS VALUES FOR DYNAMIC TEMPLATE DESIGN */}
            <style>{`
                :root {
                    --bg-base: #f4f7fb;
                    --bg-card: #ffffff;
            ...
            <aside className="w-[260px] bg-white dark:bg-[#0f172a] border-r border-slate-200 dark:border-slate-800 flex flex-col shadow-[4px_0_24px_rgba(0,0,0,0.02)] dark:shadow-[4px_0_24px_rgba(0,0,0,0.2)] z-10 relative transition-colors duration-300">
                ...
                <div className="p-6 border-t border-slate-100 dark:border-slate-800">
                    <button onClick={handleLogOutClick} className="group flex items-center w-full px-4 py-3 text-sm font-semibold text-slate-500 dark:text-slate-400 rounded-xl hover:bg-red-50 dark:hover:bg-red-900/20 hover:text-red-600 dark:hover:text-red-400 transition-all duration-200 cursor-pointer">
                ...
                <header className="h-20 bg-white/70 dark:bg-[#0f172a]/80 backdrop-blur-md border-b border-slate-200 dark:border-slate-800 flex items-center justify-between px-10 sticky top-0 z-20 transition-colors duration-300">
                    ...
                    <div className="flex items-center gap-6">
                        ...
                        <button
                            ...
                            className="p-2.5 text-slate-400 hover:text-blue-500 hover:bg-blue-50 dark:hover:bg-slate-800 transition-all rounded-full cursor-pointer"
                        >
                            {isDark ? <Sun className="h-5 w-5" /> : <Moon className="h-5 w-5" />}
                        </button>

                        <button className="relative p-2.5 text-slate-400 hover:text-slate-600 dark:hover:text-white transition-colors rounded-full hover:bg-slate-100 dark:hover:bg-slate-800 cursor-pointer">
